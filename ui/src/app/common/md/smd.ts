...
export function token_to_string(type: Token): string | void {
  switch (type) {
    case DOCUMENT:
      return 'Document';
    case BLOCKQUOTE:
...
export function attr_to_html_attr(type: Attr): string | void {
  switch (type) {
    case HREF:
      return 'href';
    case SRC:
...
export const level_to_heading = (level: number): Token => {
  switch (level) {
    case 1:
      return HEADING_1;
    case 2:
...
export const heading_to_level = (token: Token): number => {
  switch (token) {
    case HEADING_1:
      return 1;
    case HEADING_2:
 ...
 * @returns {Parser      } */
export function parser(renderer: any): Parser {
 ...
 * @param   {Token } token
 * @returns {void  } */
function add_token(p: Parser, token: Token): void {
  /*
 ...
 * @returns {number} */
function idx_of_token(p: Parser, token: Token, start_idx: number): number {
...
function continue_or_add_list(p: Parser, list_token: Token) {
 ...
 * @param   {number} charcode
 * @returns {boolean} */
function is_alnum(charcode: number): boolean {
  return (
      ...
      case MAYBE_BR:
        if (pending_with_char.startsWith('<br')) {
          if (
            ...
            /* "<br/" | "<br /" */
            (char === '/' &&
              (pending_with_char.length === 4 || p.pending[p.pending.length - 1] === ' '))
          ) {
            p.pending = pending_with_char;
            continue;
          }
 ...
 * @param   {string} value
 * @returns {void  } */
export type Renderer_Set_Attr<T> = (data: T, text: Attr, value: string) => void;
...
export function default_add_token(data: Default_Renderer_Data, type: Token): void {
  let parent: Element = data.nodes[data.index]!;
...
export function default_set_attr(data: Default_Renderer_Data, type: Attr, value: string): void {
  data.nodes[data.index]!.setAttribute(attr_to_html_attr(type) as string, value);
}
...
type Logger_Renderer_Data = undefined;

type Logger_Renderer = Renderer<Logger_Renderer_Data>;
type Logger_Renderer_Add_Token = Renderer_Add_Token<Logger_Renderer_Data>;
type Logger_Renderer_End_Token = Renderer_End_Token<Logger_Renderer_Data>;
type Logger_Renderer_Add_Text = Renderer_Add_Text<Logger_Renderer_Data>;
type Logger_Renderer_Set_Attr = Renderer_Set_Attr<Logger_Renderer_Data>;
...
export function logger_end_token(data: undefined) {
  console.log('end_token');
}

/** @type {Logger_Renderer_Add_Text} */
export function logger_add_text(data: undefined, text: string) {
  console.log('add_text: "%s"', text);
}

/** @type {Logger_Renderer_Set_Attr} */
export function logger_set_attr(data: undefined, type: Attr, value: string) {
